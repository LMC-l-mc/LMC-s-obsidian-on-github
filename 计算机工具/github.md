...
# commit：
每保存一次历史版本就是一次提交，随着commit越来越多，会形成一条commit的历史链路
...
复刻，把项目保存一份到自己名下
# 协作操作：
先fork一个到自己的vs code里，动手修改代码之前一定要创建分支，这样在同步代码时不会产生冲突，而且要确保自己已经包含了母项目那边最新的代码，可以让ai帮忙同步：告诉ai母项目地址，我要你要母项目的最新改动合并进当前分支，如果有冲突让我选择怎么处理。注意是把多的分支合并进少的的分支
![[159.png]]
# GitHub Pages：
GitHub 提供的免费静态网站托管服务，让你能把代码仓库里的 HTML、CSS、JS 文件变成可访问的网页。（CSS：一种样式表文件，用来控制网页的外观和排版）（JS：JavaScript脚本文件，专门用来控制网页的交互行为和动态效果）
# Build and deployment：
构建与部署 —— 指系统如何把你的代码“打包”并“上线”。
# Source：
来源 —— 你选择从哪个分支或文件夹发布网站。
# Deploy from a branch：
从分支部署 —— 表示你希望 GitHub Pages 从某个 Git 分支（如 main）拉取代码来生成网站。
# Branch：
分支 —— Git 中的代码版本线，main 是默认主分支。
# Select branch：
选择分支 —— 下拉菜单让你指定用哪个分支的内容来建站。
# Enforce HTTPS：
